...
time.combined <- function(filename) {
    ...
    p1 <- time.plot(df.qiime.bc) + labs(x="")
    p2 <- time.plot(df.qiime.wu) + labs(y="")
    p3 <- time.plot(df.amq) + labs(x="", y="")
...
ggsave("wu_precision.tiff", p1, width=18, height=10, units="cm")
...
ggsave("bc_precision.tiff", p2, width=18, height=10, units="cm")

p3 <- time.combined('ref_time.txt')
ggsave("ref_time.tiff", p3, width=18, height=10, units="cm")